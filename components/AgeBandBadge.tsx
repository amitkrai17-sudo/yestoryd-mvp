...
const AGE_BAND_STYLES: Record<string, { badge: string; label: string; Icon: ComponentType<{ className?: string }> }> = {
  foundation: {
...
export function AgeBandBadge({ ageBand, age, size = 'sm', showIcon = false, showEmoji }: AgeBandBadgeProps) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 rounded-full font-medium border ${style.badge} ${sizeClasses}`}>
      {(showIcon || showEmoji) && <style.Icon className={iconSize} />}
      {style.label}
    </span>
  );
}

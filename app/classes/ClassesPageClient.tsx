    ...
    >
      ...
      {classType.is_featured && (
        <span className="absolute -top-2 -right-2 bg-gradient-to-r from-[#ffde00] to-[#ff0099] text-white text-xs font-bold px-3 py-1 rounded-full shadow-md">
          <Flame className="w-3 h-3 inline" /> Popular
        </span>
      )}
      ...
      >
        ...
        {isFull ? (
          ...
          <span className="bg-[#ffde00] text-gray-900 text-xs font-bold px-3 py-1 rounded-full animate-pulse">
            <Flame className="w-3 h-3 inline" /> Only {session.spotsAvailable} left!
          </span>
        ) : (
          <span className="bg-white/20 text-white text-xs font-bold px-3 py-1 rounded-full">
        ...
        {session.book && (
          <div className="flex items-center gap-3 bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl p-3 mb-4 border border-amber-100">
            {session.book.cover_image_url ? (
              ...
              <div className="w-12 h-16 bg-amber-200 rounded-lg flex items-center justify-center">
                <BookOpen className="w-6 h-6 text-amber-700" />
              </div>
            )}
            <div className="flex-1 min-w-0">
              <p className="text-amber-700 text-xs font-semibold uppercase tracking-wide flex items-center gap-1"><BookOpen className="w-3 h-3" /> Featured Book</p>
        ...
        {session.instructor && (
          <div className="flex items-center gap-3 mb-4">
            <Avatar
              ...
              size="md"
              portal="parent"
              src={session.instructor.photo_url || undefined}
            />
            <div>
...
export default function ClassesPageClient() {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* ================================================================= */}
      {/* KAHANI TIMES VOTING SECTION */}
      {/* ================================================================= */}
      {(() => {
        const kahaniSession = sessions.find(s => s.book);
        return (
          <section className="py-12 sm:py-16">
            <div className="max-w-2xl mx-auto px-4">
              <KahaniVoting
                thisWeekBook={kahaniSession?.book || null}
                nextSessionDate={kahaniSession ? formatDate(kahaniSession.scheduledDate) : undefined}
              />
            </div>
          </section>
        );
      ...
      <section className="py-16 sm:py-20 bg-gradient-to-r from-[#ff0099] via-[#7b008b] to-[#00abff] relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 text-center">
          <div className="inline-flex items-center px-4 py-2 bg-white/20 backdrop-blur rounded-full text-white text-sm font-semibold mb-6">
            <Sparkles className="w-4 h-4 inline" /> Special Offer for Enrolled Families
          </div>

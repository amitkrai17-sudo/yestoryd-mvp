  ...
  return (
    <div className="bg-surface-0 p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex items-start justify-between gap-4 mb-6">
        ...
        <button onClick={loadData} disabled={loading} className="p-2 bg-surface-1 hover:bg-surface-2 rounded-lg text-text-tertiary flex-shrink-0">
      ...
      <div className="flex bg-surface-1 rounded-xl p-1 mb-6 overflow-x-auto">
        {TABS.map(tab => (
          <button
            ...
            className={`flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium transition-all whitespace-nowrap flex-1 sm:flex-none ${
              activeTab === tab.id
      ...
      {loading ? (
        <div className="flex justify-center py-20"><Loader2 className="w-8 h-8 text-[#FF0099] animate-spin" /></div>
      ) : (
        <>
          {activeTab === 'dashboard' && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  { label: 'Push', value: analytics?.push_sent || 0, icon: Smartphone, color: 'text-blue-400' },
                  { label: 'Email', value: analytics?.email_sent || 0, icon: Mail, color: 'text-purple-400' },
                ...
                <div key={t.id} className="bg-surface-1 border border-border rounded-xl p-3">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex gap-1 flex-shrink-0">
                      ...
                      {t.channels?.push && <Smartphone className="w-3.5 h-3.5 text-blue-400" />}
                      {t.channels?.email && <Mail className="w-3.5 h-3.5 text-purple-400" />}

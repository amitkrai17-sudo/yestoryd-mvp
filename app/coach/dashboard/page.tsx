...
function EarningsTab({ coachId }: { coachId: string }) {
  ...
  const [loading, setLoading] = useState(true);
  const [coachSplit, setCoachSplit] = useState(50);
  ...
  const fetchEarnings = async () => {
    try {
      // 1. Get program price from site_settings (single source of truth)
      ...
      // 2. Get coach data for split percentage
      const { data: coachData } = await supabase
        .from('coaches')
        .select('coach_split_percentage')
        .eq('id', coachId)
        .single();

      const defaultSplit = coachData?.coach_split_percentage || 50;
      setCoachSplit(defaultSplit);

      // 3. Get all enrolled children for this coach
      const { data: children } = await supabase
        .from('children')
        .select('id, child_name, parent_name, created_at, lead_source, custom_coach_split')
        .eq('coach_id', coachId)
        .order('created_at', { ascending: false });

      if (!children || children.length === 0) {
        setEarnings({ total: 0, thisMonth: 0, totalStudents: 0, avgPerStudent: 0, breakdown: [] });
        setLoading(false);
        return;
      }

      const coachLeadSplit = 0.70; // 70% for coach-sourced leads

      // 4. Calculate earnings for each child
      const breakdown = children.map((child) => {
        const isCoachLead = child.lead_source === 'coach';
        const splitPct = child.custom_coach_split
          ? child.custom_coach_split / 100
          : isCoachLead ? coachLeadSplit : defaultSplit / 100;
        const coachAmount = Math.round(programFee * splitPct);
        return {
          id: child.id,
          child_name: child.child_name,
          parent_name: child.parent_name || '',
          enrollment_date: child.created_at,
          split_type: isCoachLead ? '70-30' : '50-50',
          coach_amount: coachAmount,
        };
      });

      const total = breakdown.reduce((sum, e) => sum + e.coach_amount, 0);
      const now = new Date();
      const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
      const thisMonth = breakdown
      ...
      setEarnings({
        ...
        avgPerStudent: breakdown.length > 0 ? Math.round(total / breakdown.length) : 0,
        breakdown,
      });
    } catch (e) {
  ...
  const filteredBreakdown = earnings.breakdown.filter((e) => {
    if (filterPeriod === 'all') return true;
    const date = new Date(e.enrollment_date);
    const now = new Date();
    return filterPeriod === 'this_month' && date.getMonth() === now.getMonth() && date.getFullYear() === now.getFullYear();
  });

  if (loading) {
    return (<div className="flex items-center justify-center py-12"><Loader2 className="w-6 h-6 text-[#00abff] animate-spin" /></div>);
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        ...
        {filteredBreakdown.length === 0 ? (
          <div className="p-8 text-center text-gray-500">No earnings yet. Students you coach will appear here.</div>
        ) : (

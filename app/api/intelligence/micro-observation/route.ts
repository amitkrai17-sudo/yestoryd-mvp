...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { sessionId, childId, observationType, observationId, wordText, wordStatus, noteText, minutesIntoSession } = body;
...
export async function GET(request: NextRequest) {
  ...
  const { data, error } = await supabase
    .from('micro_observations')
    .select('id, observation_type, observation_id, word_text, word_status, note_text, minutes_into_session, captured_at')
